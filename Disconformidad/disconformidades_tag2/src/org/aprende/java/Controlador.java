...
public class Controlador {
	...
	public static final int NOLOGADO=0;
	...
	public void logarse(HttpServletRequest req)  {
		...
		if (user!=null) {
			...
			ses.setAttribute("Estado",this.LOGADO); //VARIABLE DE SESION con la clave el usuario
			ses.setAttribute("usuario",this.user);
		}else{
			ses.setAttribute("Estado",this.NOLOGADO); //
		}
		}catch (Exception e){
			HttpSession ses=req.getSession();
			ses.setAttribute("Estado",this.NOLOGADO);
	...
	public Iterator listarDisconformidades(Usuario usu){
		...
		Iterator it=this.listadisconf.iterator();
		return it;
	}
