...
public class  BBDD {
	...
	private void inicializa(){
		...
		try{
			...
			this.conn = DriverManager.getConnection("jdbc:mysql://localhost/disconformidad?user=alef&password=alef");
        	...
        	throw new DisconformidadException("Error desconocido" + e.getMessage());
        }finally{
        	return conectado;
        }
	}
	...
	*/
	...
	public ArrayList <Disconformidad> listadoDisconformidades(String sql){
		...
		try{
			rs = stm.executeQuery(sql);
			while (rs.next()){
				lista.add(new Disconformidad(rs.getInt("numero"), rs.getDate("fecha"),
											 rs.getString("docs"), rs.getInt("servicio"),
											 rs.getInt("idusuario"), rs.getBoolean("devolucion"),
											 rs.getString("motivo"), rs.getString("comentario")));
			}
			rs.close();
		}catch (SQLException e){
			lista = null;
