...
public class Usuario implements Comparable<Object> {
	 ...
	 * @param String con la contraseña original
	 ...
	 * @param String con la contraseña sin encriptar
	 */
	...
	public int editarUsuario(int id, String nu, String n, String a, String c, int i){
		...
		// Si el usuario no existe previamente, se modifica. Si ya existe, se envía error.
		if (arrayU.isEmpty()){
	...
	public String sqlBuscadorUsuarios ( String nU, String n, String a, String r) {
		...
		// Si el nombreUsuario no está vacío en el formulario, se busca ese nombreUsuario
		if (!nU.equals("")){
	...
	/**
	 * Esta funcion crea una consulta SQL para buscar todos NombreUsuario, Nombre y Apellidos que tengan texto
	 * que coincida con el parametro (texto) de busqueda
	 * It's created to make a generic query to search users.
	 * 
	 * @param texto buscado
	 */
	public String sqlBuscadorUsuarios (String text) {
		
		String sql = "";
		if(!text.equals("")){
			sql += " WHERE usu.nombreUsuario LIKE '%" + text + "%' OR usu.Nombre LIKE '%" + text + "%' OR usu.Apellidos LIKE '%" + text + "%'";
		}
	...
	/**
	 * Se obtiene el número total de filas de la tabla de usuarios para una búsqueda concreta
	...
	/**
	 * Para comparar dos Usuarios, se comparan sus nombreUsuario, ignorando las mayúsculas.
	 */
	...
	/**
	 * Dos Usuarios son iguales si sus nombreUsuario son iguales, ignorando las mayúsculas.
	 */
