...
public class AppletWHB extends JApplet implements ActionListener, MouseListener{
	...
	/** Etiqueta total de nÃºmero de paises. */
	...
	/** Entero RPP lÃ­mite de registros para la paginaciÃ³n consultas. */
	...
	/** Estructura de panel que almacena los mapas de google maps */
	...
	private void inicializarInterfaz(){
		...
		/*************************Aï¿½adir paneles al Applet*************************/

        /** Nuevo modulo de pestañas (jtabbedPane) */
		pestañas=new JTabbedPane();
		
		/** Jpanel del futuro mapa */
        JPanel mapa=new JPanel();
        JLabel et_mapa=new JLabel("Aqui deberia haber un mapa...");
        mapa.add(et_mapa);

        /** Código para mostrar un mapa ( Inicialmente será una mapa del mundo centrado en españa)*/
        mapsPanel = new JPanel();
        try {
            //String imageUrl = "https://maps.googleapis.com/maps/api/staticmap?center=Espa%C3%B1a&size=400x400&style=element:labels|visibility:off&style=element:geometry.stroke|visibility:off&style=feature:landscape|element:geometry|saturation:-100&style=feature:water|saturation:-100|invert_lightness:true&key=AIzaSyBvLThRlUzwhGtTOypmXwNll7Dzaj1Au6Q";
            String imageUrl = "https://maps.googleapis.com/maps/api/staticmap?center=Espa%C3%B1a&zoom=1&size=400x400&maptype=hybrid&key=AIzaSyBvLThRlUzwhGtTOypmXwNll7Dzaj1Au6Q";
        ...
        mapsPanel.add(new JLabel(new ImageIcon((new ImageIcon("image.jpg")).getImage().getScaledInstance(400, 400,
                java.awt.Image.SCALE_SMOOTH))));

        /** Se añaden los distintos jpanel al modulo de pestañas */
        pestañas.addTab("Gráfica", grafica);
        
        /** Se añade el mapa al jpanel del modulo de pestañas */
        pestañas.addTab("GoogleMpas", mapsPanel);

        /** Se añaden el jpanel de los datos a la izquiera y a la derecha el jtabbedpane para las pestañas */
        add(izquierda, BorderLayout.WEST);
        add(pestañas, BorderLayout.EAST);
	...
	/**
	 * Aï¿½adimos escuchadores de eventos a los botones y a las dos tablas.
	 */	
	...
	public void mouseClicked(MouseEvent ev) {
		...
		/**
		 * Codigo por el que cuando se produce un click de raton se realiza una modificacion
		 * del mapa capturando el pais seleccionado y mostrandolo el el jpanel 
		 */
		String Pais = (String) tablaPaises.getValueAt(filaPaises, 1);
		System.out.println("\n\n\n***********************Pais: "+Pais);
		String PaisModif = Pais.replace(" ", "%20");
		int indice = PaisModif.indexOf('(');
		if (indice != -1) {
			PaisModif = PaisModif.substring(0, indice);
		}
		
		try {
            String imageUrl = "https://maps.googleapis.com/maps/api/staticmap?center=" + PaisModif + "&zoom=5&size=400x400&maptype=hybrid&key=AIzaSyBvLThRlUzwhGtTOypmXwNll7Dzaj1Au6Q";
		...
		mapsPanel.removeAll();
        mapsPanel.add(new JLabel(new ImageIcon((new ImageIcon("image2.jpg")).getImage().getScaledInstance(400, 400,
                java.awt.Image.SCALE_SMOOTH))));
	...
	/**
	 * Metodo que aï¿½ade un arrayList de Paises y su correspondiente filtro a su tabla correspondiente.
	 *
	...
	/**
	 * Metodo que aï¿½ade un arrayList de Indicadores y su correspondiente filtro a su tabla correspondiente.
	 *
	...
	/**
	 * Metodo que aï¿½ade un arrayList de Estadisticas y su correspondiente filtro a su tabla correspondiente.
	 *
	...
	public void setNumResultados(int numResultados){
		System.out.print("Ponemos el nÃºmero total de registros de resultados.");
	...
	public void initPagPaises(){
		...
		setNumPagPaises(pagActualPaises,totalPagPaises);
		//Avanzamos una vez para dibujar valores de la primera pÃ¡gina
		if(registroPaisActual<=totalRegistrosPaises-RPP){
			modeloPaises.removeAllRows();
			System.out.println("registroPaisActual+RPP: "+(registroPaisActual+RPP)+" registroPaisActual: "+registroPaisActual);
			cliente.obtenerPaises(registroPaisActual+RPP, registroPaisActual);
