  ...
  //Método que hará que me de solo el número de filas que cumplen las condiciones
    ...
    public static int calculaNumeroPaginas(String campo, String texto) {
        ...
        if (texto != null && !texto.equals("")) {
            sql = "SELECT * FROM MESAS WHERE " + campo + " like '%" + texto + "%'";
        }
        listaMesas = ejecutaSQLMesa(sql);
        if ((listaMesas.size() % INCREMENTO) == 0) {
            return (listaMesas.size() / INCREMENTO) - 1;
        } else {
            return listaMesas.size() / INCREMENTO;
        }
    }
