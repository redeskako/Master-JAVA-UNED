 ...
 * visualización de las tareas de un usuario-responsable: 
 * - Tabla de reservas pendientes, para confirmar o denegar 
 * - Tabla de reservas pendientes de anulación, para anular o no anular
 * 
...
public class Tareas extends HttpServlet {
	 ...
	 * correspondiente.
	 * 
	 * @param request con el contenido del href
	...
	protected void doGet(HttpServletRequest request,
		...
		case "peticiones":
			...
			RequestDispatcher rd = request.getRequestDispatcher("tareas_PeticionesPendientes.jsp");
		...
		case "reservas":
			...
			// Se re-dirige a la página jsp:
			RequestDispatcher rd2 = request.getRequestDispatcher("tareas_ReservasPendientes.jsp");
		...
		case "confirmar":
			...
			// ido bien.
			if (idConfirmado != 0) {
				response.sendRedirect("./Tareas?tipo=peticiones&nombre=" + nombre 
						+ "&idEditado=" + idConfirmado + "&accion=confirmada");
				System.out.println("Se ha confirmado la reserva con ID "+idConfirmado);
			} else {
		...
		case "denegar":
			...
			if (idDenegado != 0) {
				response.sendRedirect("./Tareas?tipo=peticiones&nombre=" + nombre 
						+ "&idEditado=" + idDenegado + "&accion=denegada");
				System.out.println("Se ha denegado la reserva con ID "+idDenegado);
			} else {
				response.sendRedirect("./Tareas?tipo=peticiones&idEditado=0");
		...
		case "anular":
			...
			if (idAnulado != 0) {
				...
				System.out.println("Se ha anulado la reserva con ID "+idAnulado);
			} else {
		...
		case "noAnular":
			...
			// Se solicita el cambio de estado a no anulado (6) y se almacena el id:
			...
			if (idNoAnulado != 0) {
				...
				System.out.println("No se ha anulado la reserva con ID "+idNoAnulado);
			} else {
				response.sendRedirect("./Tareas?tipo=reservas&idEditado=0");
