...
public class AccionesRecursos extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		case "listado":
			...
			RequestDispatcher rd = request.getRequestDispatcher("recursosListado.jsp");
			rd.forward(request, response);

			break;

		// Si se ha pulsado en el menú: crear recurso
		case "menuCrear":
			// Se re-dirige a la página jsp:
		...
		case "crear":
			...
			if (idNuevo != 0) {
				...
				System.out.println("Se ha creado un nuevo recurso con ID "+idNuevo+" y descripción '"+descripcion+"'");
			} else {
		...
		// Si se ha pulsado en el menú: editar/eliminar recurso
		case "menuEditar":
			...
			// Se re-dirige a la página jsp:
			RequestDispatcher rd3 = request.getRequestDispatcher("recursosEditar.jsp?idEditado="
		...
		case "botonEditar":
			...
			// Se re-dirige:
			response.sendRedirect("./AccionesRecursos?tipo=menuEditar&idEditar=" + idListaEditar);
		...
		case "editar":
			...
			// ido bien.
			if (idEditado != 0) {
				response.sendRedirect("./AccionesRecursos?tipo=menuEditar&nombre=" + descripcionEd 
						+ "&idEditado=" + idEditado);
				System.out.println("Se ha actualizado un recurso con ID "+idEditado);
			} else {
				response.sendRedirect("./AccionesRecursos?tipo=menuEditar&idEditado=0");
				System.out.println("ERROR actualizando recurso con ID "+idRecurso);
			}
		...
		case "botonEliminar":
			...
			// Se re-dirige:
			response.sendRedirect("./AccionesRecursos?tipo=menuEditar&idEliminar=" + idListaEliminar
		...
		case "eliminar":
			...
			// ha ido bien.
			response.sendRedirect("./AccionesRecursos?tipo=menuEditar&eliminado=" + eliminado
					+ "&nombre=" + nombre);
			System.out.println("Se ha eliminado un recurso con ID "+idEliminar);
		...
		case "buscador":
			...
			// Se re-dirige a la página jsp:
			RequestDispatcher rd4 = request.getRequestDispatcher("recursosBuscador.jsp?busqueda=" + busqueda);
