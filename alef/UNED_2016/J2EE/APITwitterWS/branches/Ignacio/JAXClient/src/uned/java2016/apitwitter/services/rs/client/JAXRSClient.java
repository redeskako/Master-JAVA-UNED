...
import uned.java2016.apitwitter.services.rs.client.entity.NeighbourMessageBodyReader;
import uned.java2016.apitwitter.services.rs.client.entity.EstudioMessageBodyReader;
import uned.java2016.apitwitter.services.rs.client.entity.EstudiosMessageBodyReader;
...
public class JAXRSClient {
	...
	public static final String OPERATION_GETSTUDIESBYHASHTAG="estudios/getByHashtag";
	public static final String OPERATION_GETBYNCT="estudios/getByNCT";
	public static final String OPERATION_GETNEIGHBOUR="neighbour/getNeighbour";
	...
	protected void initialize(){
		...
		Client c=ClientBuilder.newBuilder()
				  ...
				  .register(EstudioMessageBodyReader.class) // provider para instancias de ClinicalStudy
				  .register(EstudiosMessageBodyReader.class) //provider para List<ClinicalStudy>
				  .register(NeighbourMessageBodyReader.class) //provider para List<String> Vecinos
				  .build();
		this.setRsClient(c);		
	}
	...
	/**
	 * implementa la llamada a la operacion 'getByNCT' del servicio 'estudios'
	 * @param URI URL contra el servicio web 
	 * @param nct identificador del estudio
	 * @return Instancia de ClinicalStudy.
	 */
	
	public ClinicalStudy getByNCT(String URI,String nct){
		ClinicalStudy ret=null;
		// empezamos a construir la llamada
		WebTarget t=this.getRsClient()
				  .target(URI)				  
				  .path(JAXRSClient.OPERATION_GETBYNCT)
				  .path(nct);
		// realizamos la llamada y recuperamos el estudio. Al indicar la clase 'ClinicalStudy', la infraestructura
		// utilizara una instancia de ClinicalStudyMessageBodyReader para realizar la de-serializacion
		ret=(ClinicalStudy) this.invoke(t,ClinicalStudy.class);
		System.out.println("EL NCT ES:"+ret.getNctId());
		return ret;
	}
	...
	/**
	 * Invoca la operacion 'getByHashtag' del servicio 'estudios', indicando en 'hasthag' el hashtag
	 * por el que buscar y en 'count' el numero de estudios a devolver.
	 * @param URI URL contra el servicio 
	 * @param hashtag Hasthag por el que buscar tweets
	 * @param count numero de estudios a recuperar en la respuesta
	 * @return
	 */
	public ClinicalStudy[] getStudiesByHashtag(String URI,String hashtag,int count){
		ClinicalStudy[] ret=null;
		// empezamos a construir la llamada
		WebTarget t=this.getRsClient()
				  .target(URI)				  
				  .path(JAXRSClient.OPERATION_GETSTUDIESBYHASHTAG)
				  .path(hashtag)
		...
		List<ClinicalStudy> tt=(List<ClinicalStudy>) this.invoke(t,new GenericType<List<ClinicalStudy>>(){});
		// Convertimos la lista en array.
		ret=tt.toArray(new ClinicalStudy[0]);
		return ret;
	}	
	
	/**
	 * Invoca la operacion 'getNeigbour' del servicio 'neighbour', indicando en 'hasthag' el hashtag
	 * por el que buscar
	 * @param URI URL contra el servicio 
	 * @param hashtag Hasthag por el que buscar vecinos
	 * @return
	 */

	public String[] getNeigbour(String URI,String hashtag){
		String[] ret=null;
		// empezamos a construir la llamada
		WebTarget t=this.getRsClient()
				  .target(URI)				  
				  .path(JAXRSClient.OPERATION_GETNEIGHBOUR)
				  .path(hashtag);
		// invocamos el WS para recuperar un tipo generico de tipo List<Tweet> que es el que devuelve
		// e la operacion getByHashTag
		List<String> tt=(List<String>) this.invoke(t,new GenericType<List<String>>(){});
		// Convertimos la lista en array.
		try{
		ret=tt.toArray(new String[0]);
	...
	/**
	 * Llama al WS apuntado por 'target' e intenta parsear la respuesta como un tipo generico 'gen'
	 * En este caso lo usamos para recuperar un array de estudios
