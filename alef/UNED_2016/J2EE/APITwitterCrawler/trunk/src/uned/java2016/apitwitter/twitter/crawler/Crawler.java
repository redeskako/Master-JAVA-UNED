...
public class Crawler {
	...
	public ExecutionResult doProcess(ExecutionRequest init)
	  ...
	  // procesamos todos los hashtag's, parando si es necesario
	  while(st.hasMoreTokens() && this.processHashtag(poll,st.nextToken()));
	 ...
	 * @return boolean true si todo ha ido bien, false si se debe parar el sondeo (http 429)
	 */
	protected boolean processHashtag(Poll poll,String hashtag){
		...
		// flag para escapar en caso de error 429
		boolean continuar=true;
		while(continuar && it.hasNext())
		{
		   h=it.next();
		   ...
		   if(!h.isProcessed())
		   {
			   ...
			   this.getAllTweets(poll, this.getSearchRequest(), h , ttParser);
				   ...
				   // en caso de error, comprobamos si es un 429
				   if(e instanceof TwitterRestAPIException &&
					  ((TwitterRestAPIException) e).getHttpResponseCode() == 429)
					   continuar=false;
			   }
			   // ...
			   ...
			   {
				   // marcamos como procesados todos los hashtag's que quedan, ya que
				   // salimos con error
				   while(it.hasNext())
					   it.next().setProcessed(true);
				   // logamos q salimos por codigo 429
				   this.logger.error("Abortamos ejecucion por codigo HTTP 429");
			   }
		   }
