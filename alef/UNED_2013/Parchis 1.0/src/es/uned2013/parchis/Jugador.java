...
public class Jugador {
	...
	public int tirarDado(boolean esInicio) {
		...
		case NORMAL:Random azar = new Random();
					tirada = azar.nextInt(6) + 1;
					break;
		case PRUEBA:if (Parchis.listaTestTirada.hasNext())
		...
		Parchis.getUI().mostrarMensajeString(ParchisI18Keys.JUGADORENTURNO, nombre);
		if (Parchis.getModo() == Modo.NORMAL) Parchis.getUI().solicitarCadena(ParchisI18Keys.CAMBIARTURNO);
			...
			if (tirada == 6 || tirada == 7) {
				...
				if (seises == 3) {
					...
					if (segundoSeis.getCasillaActual() < 
						...
						tablero.fichaComida(segundoSeis);
						Parchis.getUI().mostrarMensaje(ParchisI18Keys.DEVUELVOCASA);
					}
			...
			if (fichasMovibles.size() > 0) {
				if (tirada == 5) {
					...
					if (fic != null) {
						...
						fichaSacada = true;
						Parchis.getUI().mostrarMensajeInteger(ParchisI18Keys.SACARFICHA, fic.getIdentificador());
					}
				}
				if (tirada == 6 || tirada == 7) {
				...
				if (!fichaSacada) {
					...
					Parchis.getUI().mostrarMensajeInteger(ParchisI18Keys.MOVIENDOFICHA, fic.getIdentificador());
				}
	...
	private TreeSet<Integer> calculaBarreras(TreeMap<Integer, Ficha> fichasMovibles) {
		...
		if (barreras.size() > 0) {
			if (barreras.size() > 1)
				Parchis.getUI().mostrarMensajeString(ParchisI18Keys.BARRERASPENDIENTES,cad);
			else
				Parchis.getUI().mostrarMensajeString(ParchisI18Keys.BARRERAPENDIENTE,cad);
		}
	...
	private Ficha seleccionaFicha(TreeMap<Integer, Ficha> fichasMovibles) {
		...
		if (cantFichas > 0) {
			...
			if (cantFichas > 1) 
				Parchis.getUI().mostrarMensajeString(ParchisI18Keys.SOLOMOVERFICHASID, cad);
			else
				Parchis.getUI().mostrarMensajeString(ParchisI18Keys.SOLOMOVERFICHA, cad);
			...
			if (cantFichas > 1) { 
				...
				while (!valido) {
					switch(Parchis.getModo()) {
				   	case NORMAL:
				   		idFicha = Parchis.getUI().solicitarEntero(ParchisI18Keys.INTRODUZCAIDFICHA);
							break;
						case PRUEBA:
					...
					if (fichasMovibles.containsKey(idFicha)) {
						...
						Parchis.getUI().mostrarMensaje(ParchisI18Keys.ERRORVALORNOPOSIBLE);
					}
				}
			}
			else {
