...
import es.uned.master.java.healthworldbank.servidor.ServidorGUI;
...
public class ClienteBD {
	...
	public void obtenerPaises(int limite, int primerRegistro) {
		try {
			...
			/** Traza con informacion de la pregunta por paises que hace el cliente */
			System.out.println("pregunta = " + TipoPeticion.PAISES + ", limite = " + limite);
	...
	public void obtenerIndicadores(int limite, int primerRegistro) {
		try {
			...
			/** Traza con informacion de la pregunta por indicadores que hace el cliente */
			System.out.println("pregunta = " + TipoPeticion.INDICADORES + ", limite = " + limite);
	...
	private Socket conectarAServidor(Socket cliente) throws IOException {
		...
		try {
			...
			applet.mostrarError(ex.getMessage(),"Error de conexion: ");
