 ...
 * @date 2017 04.
 */
@WebServlet(name = "TestAjax" , urlPatterns = {"/main/users/statistics"})
public class StatisticsController extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        } else if(operation != null && operation.equals("country-list")){
            // Se encarga de hacer la búsqueda de texto para el autocomplete del país
            String country = req.getParameter("country");
            try{
                dataBean.getCountriesList(country);
                resp.setContentType("text/plain;charset=UTF-8");
                PrintWriter out = resp.getWriter();
                String jsonCountries = dataBean.objectToJson(dataBean.getCountrySearchContainer());
                out.println(jsonCountries);
                out.close();
            } catch (ServerRunningException e){
                e.printStackTrace();
