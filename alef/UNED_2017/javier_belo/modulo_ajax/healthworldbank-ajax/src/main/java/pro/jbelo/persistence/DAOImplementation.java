...
public class DAOImplementation implements Serializable, DAO {
    ...
    /**
     * Realiza la búsqueda de países segun el texto pasado usado ILIKE
     * @param country
     * @return
     * @throws ServerRunningException
     */
    public List<CountryEntity> readCountries(String country) throws ServerRunningException {

        StringBuilder sql = new StringBuilder("SELECT * FROM country WHERE country_name ILIKE ");
        sql.append("'%");
        sql.append(country);
        sql.append("%';");
        ...
        List<CountryEntity> dataEntityList = session.createNativeQuery(sql.toString(),CountryEntity.class).getResultList();
