...
public class DataBean implements Serializable{
     ...
     */
    private final ArrayList<DataMapWrapper> dataMapEntities = new ArrayList<>();

    private final EntitiesContainer countrySearchContainer = new EntitiesContainer();

    public EntitiesContainer getCountrySearchContainer() {
     ...
     */
    public synchronized ArrayList<DataMapWrapper> getDataMapEntities() {
        return dataMapEntities;
    }
    ...
    public synchronized void retrieveCountries(String action) throws ServerRunningException {
        ...
        try {
            ...
            List<CountryEntity> countryList = daoImplementation.readCountries(RECORDS_PER_PAGE,newOffset);
    ...
    /**
     * Se encarga de la petición de países basados en una cadena de texto
     * @param countryHint texto usaod para buscar países
     * @throws ServerRunningException
     */
    public synchronized void getCountriesList(String countryHint) throws ServerRunningException {

        try{
            List<CountryEntity> countryEntityList = daoImplementation.readCountries(countryHint);

            countrySearchContainer.getEntities().clear();

            for(CountryEntity c: countryEntityList){
                CountryEntity reg = new CountryEntity();
                reg.setCountryCode(c.getCountryCode());
                reg.setCountryName(c.getCountryName());
                countrySearchContainer.getEntities().add(reg);
            }
        } catch (ServerRunningException e){
            throw new ServerRunningException(e.getLocalizedMessage());
        }
    ...
    public synchronized void retrieveIndicators(String  action) throws ServerRunningException {
        ...
        try {
            ...
            List<HealthIndicatorEntity> mList = daoImplementation.readHealthIndicators(RECORDS_PER_PAGE,newOffset);
    ...
    public synchronized void retrieveStatistics(String  action, String countryCode, String indicatorCode) throws ServerRunningException {
        ...
        try{
            ...
            List<DataEntity> mList = daoImplementation.readData(countryCode,indicatorCode,newOffset,RECORDS_PER_PAGE);
    ...
    public synchronized void retrieveGeoStatistic(String indicator, String year) throws ServerRunningException{
        ...
        try {
            List<DataMapWrapper> mList = daoImplementation.requestGeoStatistics(indicator,year);

            dataMapEntities.clear();

            for (DataMapWrapper d: mList){
    ...
    public synchronized String objectToJson(Object object){
        // TODO modify this and use JSR 374
        String objectJson = "";
