...
public class AccionesReservas extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		case "listado":
			...
			RequestDispatcher rd1 = request.getRequestDispatcher("reservasListado.jsp");
		...
		case "menuCrear":
			...
			// Se re-dirige a la página jsp:
			RequestDispatcher rd2 = request.getRequestDispatcher("reservasCrear.jsp?idNuevo="
					+ idNuevoS);
		...
		case "crear":
			...
			if (idNuevo != 0) {
				...
				response.sendRedirect("./AccionesReservas?tipo=menuCrear&idNuevo="
						+ idNuevo + "&nombre=" + n);
				System.out.println("Se ha creado una nueva reserva con ID "+idNuevo+" y hecho por '"+n+"'");
			} else {
		...
		// Si se ha pulsado en el menú: editar/eliminar reserva
		case "menuEditar":
			...
			// Se re-dirige a la página jsp:
			RequestDispatcher rd3 = request.getRequestDispatcher("reservasEditar.jsp?idEditado="
					+ idEditadoS + "&eliminado=" + eliminadoS+ "&nombre=" + nombre);
		...
		case "botonEditar":
			...
			// Se re-dirige:
			response.sendRedirect("./AccionesReservas?tipo=menuEditar&idEditar=" + idListaEditar);
		...
		case "editar":
			...
			try {
				...
				idEditado = r.editarReserva(idReserva, usuIdEdit, recIdEdit,
						inicioEdit, finEdit, estadoIdEdit);
			...
			if (idEditado != 0) {
				...
				System.out.println("Se ha actualizado una reserva con ID "+idEditado);
			} else {
				response.sendRedirect("./AccionesReservas?tipo=menuEditar&idEditado=0");
				System.out.println("ERROR actualizando reserva con ID "+idReserva);
			}
		...
		case "eliminar":
			...
			System.out.println("Se ha eliminado una reserva con ID "+idEliminar);
			break;
		...
		case "buscador":
			...
			if (request.getParameter("inicioD") != null && !request.getParameter("inicioD").equals("")) {
				try {
			...
			if (request.getParameter("inicioH") != null && !request.getParameter("inicioH").equals("")) {
				try {
			...
			if (request.getParameter("finalD") != null && !request.getParameter("finalD").equals("")) {
				try {
			...
			if (request.getParameter("finalH") != null && !request.getParameter("finalH").equals("")) {
				try {
			...
			// Se añaden los datos del formulario, para que se muestren por
			// defecto:
			...
			String url = "reservasBuscador.jsp?usuario=" + usuB + "&recurso="
					+ recB + "&estado=" + estB + "&inicioD=" + stringID
					+ "&inicioH=" + stringIH + "&finalD=" + stringFD
					+ "&finalH=" + stringFH;
