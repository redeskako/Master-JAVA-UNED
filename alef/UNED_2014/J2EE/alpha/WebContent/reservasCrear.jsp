...
<body>
	<%
		// Si ha accedido un responsable en vista de responsable o empleado: se muestra la pagina
		Integer rolSesion = (Integer) session.getAttribute("rolSesion");
		Integer vistaSesion = (Integer) session.getAttribute("vistaSesion");
		String usuarioSesion = (String) session.getAttribute("idSesion");

		if (rolSesion != null && (vistaSesion == 1 || vistaSesion == 2)) {
	%>
	<div id="container">
		...
		<div id="content">
			...
			<form name="formCrearReserva" action="./AccionesReservas" method="get" onsubmit="return controlForm();">
				<div class="cntCampo">
					...
					<% 
					// Se muestra el combo sólo para responsables(2):
					if(vistaSesion == 2){
					%>
					<div>
					...
					<% 
						...
						// Si es empleado, se ve el campo fijo con su nombreUsuario	
						Usuario u2 = new Usuario();
						int idUsuario = u2.getIdUsuario(usuarioSesion);
					%>	
					<div>
						<select name="usuario">
							<option value="<%=idUsuario%>"> <%= usuarioSesion %></option>
						</select>
					</div>
					<% 
					}
				...
				<div class="cntCampo">
					...
					<% 
						...
						// Si es empleado, se ve el campo fijo con estado pendiente(1)	
					%>	
					<div>
						<select name="estado">
