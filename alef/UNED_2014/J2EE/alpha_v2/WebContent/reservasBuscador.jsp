			...
			<div class="paginacion">
				...
				<ul>
					<c:forEach var="p" begin="1" end="${ paginas }">
						<li><a href="./AccionesReservas?tipo=buscador&p=<c:out value="${ p }" />
							&usuario=<c:out value="${ usuarioB }" />&recurso=<c:out value="${ recursoB }" />
							&estado=<c:out value="${ estadoB }" />&inicioD=<c:out value="${ inicioD }" />
							&inicioH=<c:out value="${ inicioH }" />&finalD=<c:out value="${ finalD }" />
							&finalH=<c:out value="${ finalH }" />"><c:out value="${ p }" /></a></li>
					</c:forEach>
				</ul>
			</div>
			</c:if>
