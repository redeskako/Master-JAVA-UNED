		...
		try {
			...
			while ( eventReader.hasNext() ) {
				...
				if ( event.isStartElement() ) {
					...
					if ( event.asStartElement().getName().getLocalPart().equals( X ) ) {
						event = eventReader.nextEvent();
						casillaInfo.setCoordenadaX( Integer.parseInt( event.asCharacters().getData() ) );
						continue;
					}
					if ( event.asStartElement().getName().getLocalPart().equals( Y ) ) {
						event = eventReader.nextEvent();
						casillaInfo.setCoordenadaY( Integer.parseInt( event.asCharacters().getData() ) );
						continue;
					}
				}
				// añadimos casila a la lista
