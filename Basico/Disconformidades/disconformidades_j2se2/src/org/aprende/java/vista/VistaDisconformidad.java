...
public void llenarcombo (Servicios s){
    	
        for (Iterator it=s.iterator(); it.hasNext( ); ) { 
             servicioorigen.addItem(((Servicio)it.next()).nombre());
          }
    }

public void llenarcomboUsuarios (Usuarios s){
	
    for (Iterator it=s.iterator(); it.hasNext( ); ) { 
         usuario.addItem(((Usuario)it.next()).nombre());
     }
}
    
    
	public void  anteriordisponible( boolean valor){
    	this.btnAnterior.enable(valor);
    }
    
//    public void setvalores(Disconformidad d, Servicios s, Usuarios u){
   	public void setvalores(Disconformidad d, String s, String u){    	
    	this.documentosafectados.setText(d.docs());
   	 	this.motivo.setText(d.motivo());
   	 	this.comentario.setText(d.comentario());
   	 	this.devolucionsi_no.setSelected(d.devolucion());
   	 	
   	 	//System.out.println(d.servicio());
   	 	//System.out.println(s.getServicio(d.servicio()).nombre());
   	 	
//   	 	this.servicioorigen.setSelectedItem(s.getServicio(d.servicio()).nombre());
//   	    this.usuario.setSelectedItem(u.getUsuario(d.usuario()).nombre());
   	 	this.servicioorigen.setSelectedItem(s);
   	    this.usuario.setSelectedItem(u);
   	    
   	 	//this.servicioorigen.setSelectedIndex(d.servicio()-1);
   	    
    }
    
    public Disconformidad getvalores(Disconformidad d, Servicios s, Usuarios u){
    	Date hoy = new 	Date();
    	d.docs(this.documentosafectados.getText());
    	d.motivo(this.motivo.getText());
    	d.comentario(this.comentario.getText());
    	d.devolucion(this.devolucionsi_no.isSelected());
   	 	d.fecha(hoy);
   	 	//d.numero(1);
   	 	d.servicio(s.getIdServicio((String) servicioorigen.getSelectedItem()));
   	 	d.usuario(u.getIdUsuario((String) usuario.getSelectedItem()));
   	 	//d.servicio(s.getIdServicio("Servicio 3"));
   	 	//d.usuario(1);
    ...
    private void estableceCamposActivos(int estado){
		...
		if (estado == org.aprende.java.controlador.Controlador.EDICION){
			...
			this.lblEstado.setText("ESTADO: Edici�n");
    ...
    //establece los componentes del formulario que est�n activos y los que no dependiendo del estado
    ...
    //m�todo que comprueba el formato de los datos introducidos
