...
/**
 * Servlet implementation class AccionesUsuarios Gestiona las acciones a
 * realizar relacionadas con los usuarios en función del parámetro "tipo" del
	...
	protected void doGet(HttpServletRequest request,
		...
		case "listado":
			...
			RequestDispatcher rd1 = request
					.getRequestDispatcher("usuariosListado.jsp");
		...
		case "menuCrear":
			...
			RequestDispatcher rd2 = request
					.getRequestDispatcher("usuariosCrear.jsp?idNuevo="
							+ idNuevoS + "&nombre=" + nombre);
		...
		case "crear":
			...
			int idNuevo = u.nuevo(nombreUsuario, nom, apellidos, contrasena,
					idRol);
			...
			// Si el nuevo id es 0, ha habido un error al crear el usuario. Si
			// no, todo ha ido bien.
			if (idNuevo != 0) {
				response.sendRedirect("./AccionesUsuarios?tipo=menuCrear&idNuevo="
						+ idNuevo + "&nombre=" + nombreUsuario);
				System.out.println("Se ha creado un nuevo usuario con ID "
						+ idNuevo + " y NombreUsuario '" + nombreUsuario + "'");
			} else {
				response.sendRedirect("./AccionesUsuarios?tipo=menuCrear&idNuevo=0");
				System.out
						.println("ERROR al crear Usuario: es posible que exista otro usuario con el mismo nombreUsuario.");
			}
		...
		case "menuEditar":
			...
			RequestDispatcher rd3 = request
					.getRequestDispatcher("usuariosEditar.jsp?idEditado="
		...
		case "botonEditar":
			...
			// Se re-dirige:
			response.sendRedirect("./AccionesUsuarios?tipo=menuEditar&idEditar="
					+ idListaEditar);
		...
		case "editar":
			...
			// Se crea el nuevo usuario y se recupera su id:
			int idEditado = u.editarUsuario(idUsuario, nombreUsuarioEd,
					nombreEd, apellidosEd, contrasenaEd, idRolEd);

			// Si el nuevo id es 0, ha habido un error al editar. Si no, todo ha
			// ido bien.
			if (idEditado != 0) {
				response.sendRedirect("./AccionesUsuarios?tipo=menuEditar&nombre="
						+ nombreUsuarioEd + "&idEditado=" + idEditado);
				System.out.println("Se ha actualizado un usuario con ID "
						+ idUsuario);
			} else {
				response.sendRedirect("./AccionesUsuarios?tipo=menuEditar&idEditado=0");
				System.out
						.println("ERROR actualizando usuario con ID "
								+ idUsuario
								+ ": es posible que exista otro usuario con el mismo nombreUsuario.");
			}
		...
		case "botonEliminar":
			...
			// Se re-dirige:
			response.sendRedirect("./AccionesUsuarios?tipo=menuEditar&idEliminar="
		...
		case "eliminar":
			...
			if (idEliminar == idU) {
				System.out.println("Se ha eliminado un usuario con ID "
						+ idEliminar);
				response.sendRedirect("./SalidaUsuario?eliminado=" + eliminado);
			} else {
				// Si eliminado == true, se ha eliminado sin problema. Si no,
				// todo ha ido bien.
				System.out.println("Se ha eliminado un usuario con ID "
						+ idEliminar);
				response.sendRedirect("./AccionesUsuarios?tipo=menuEditar&eliminado="
		...
		case "buscador":
			...
			if (request.getParameter("nombreUsuarioB") != null
					&& !request.getParameter("nombreUsuarioB").equals("")) {
				nombreUsuarioB = request.getParameter("nombreUsuarioB");
			} // fin if

			if (request.getParameter("nombreB") != null
					&& !request.getParameter("nombreB").equals("")) {
				nombreB = request.getParameter("nombreB");
			} // fin if

			if (request.getParameter("apellidosB") != null
					&& !request.getParameter("apellidosB").equals("")) {
				apellidosB = request.getParameter("apellidosB");
			} // fin if

			if (request.getParameter("rolB") != null
					&& !request.getParameter("rolB").equals("0")) {
				rolB = request.getParameter("rolB");
			} // fin if

			// Se buscan los usuarios que cumplen los requisitos:
			String sql = u.sqlBuscadorUsuarios(nombreUsuarioB, nombreB,
					apellidosB, rolB);
			...
			// Se re-dirige a la página jsp:
			String url = "usuariosBuscador.jsp?nombreUsuarioB="
					+ nombreUsuarioB + "&nombreB=" + nombreB + "&apellidosB="
					+ apellidosB + "&rolB=" + rolB;
