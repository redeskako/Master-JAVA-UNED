...
public class Reserva implements Comparable<Object> {
	...
	/**
	 * Método constructor, asigna valor a las variables de la tabla Reservas en
	 * BBDD:
	 * 
	 * @param idres
	 *            para idReserva
	...
	/**
	 * Se obtiene el número total de filas de la tabla de reservas de un
	 * usuario.
	 * 
	 * @param usuario
	...
	/**
	 * Se obtiene el número total de filas de la tabla de reservas con un estado
	 * concreto.
	 * 
	 * @param idEstado
	...
	public ArrayList<Reserva> reservasListado(int pagina, int i) {
		...
		// Si no se ha elegido el listado completo (pagina = 0), se añade la
		// paginación:
		if (pagina != 0) {
			sql += " LIMIT " + limit + ",20 ";
		}
	...
	public int nuevo(int usu, int rec, Date i, Date f, int est, int suc) {
		...
		// Si el usuario no existe previamente, se crea. Si ya existe, se envÃ­a
		// error.
		if (comprobada) {
			BaseDatos bbdd = new BaseDatos();
			bbdd.abrirConexion();
	...
	public int editarReserva(int id, int idU, int idRec, Date i, Date f,
		...
		// Si la reserva es compatible, se modifica. Si ya existe, se envÃ­a
		// error.
		if (comprobada) {
			String fi = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
					.format(inicio);
	 ...
	 * anulado, pendiente de anulación), que requiere comprobar compatibilidad.
	 * 
	 * @param idReserva
	 *            de la reserva
	 * @param idRecurso
	...
	public String sqlBuscadorReservas(int vista, int u, int r, Date iniD,
		...
		// Se obtiene una fecha "vacÃ­a" para poder comprobar si se han
		...
		if (u != 0) {
			...
			// Si el nombreUsuario estÃ¡ vacÃ­o en el formulario, se busca en
	 ...
	 * Se ordenan por fecha de inicio primero, después por id. Dos reservas con
