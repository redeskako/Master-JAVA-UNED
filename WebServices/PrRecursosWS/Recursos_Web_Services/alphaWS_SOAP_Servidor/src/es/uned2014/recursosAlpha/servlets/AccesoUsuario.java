...
/**
 * Servlet implementation class AccesoUsuario. Se ejecuta cuando el usuario
 * envía el formulario de acceso a la aplicación. Si el formulario se ha
 * cumplimentado, se comprueba si existe un usuario con esos nombre de usuario y
 * contraseña en la base de datos. Si es así, se accede en calidad de empleado o
 * responsable, según sea su rol.
 * 
...
public class AccesoUsuario extends HttpServlet {
	...
	/**
	 * Método en el que se describen las acciones a realizar cuando un usuario
	 * intenta acceder a través del formulario de acceso. 1 Se recuperan los
	 * valores de usuario y contraseña 2 Si los campos no están vacíos: se busca
	 * si existe en BBDD. 2.1 Si existe, se crea cookie y sesión. Se envía al
	 * menú correspondiente. 2.2 Si no existe se vuelve a index con estado no
	 * válido 3 Si algún campo está vacío: vuelve a index.jsp con estado no
	 * válido
	 * 
	 * @param request
	...
	protected void doService(HttpServletRequest request,
		...
		if (usuario != null && contrasena != null) {
			// Se crea un usuario como respuesta de comprobar los valores
			// enviados
			...
			if (u != null) {
				System.out.println("Ha accedido el usuario "
						+ u.getNombreUsuario());
				...
				System.out
						.println("La cookie y la sesión se han creado correctamente");

				// Se almacena el rol del usuario en la sesión
				int rol = u.getIdRol();
				session.setAttribute("rolSesion", rol);

				// Se almacena la vista de la sesión (para rol responsable, que
				// tiene acceso a las dos)
				...
				// Se carga index.jsp en visión empleado o responsable, en
				// función del rol
				if (rol == 2) {
			...
			System.out
					.println("Usuario no válido: no se han rellenado los campos del formulario.");
			response.sendRedirect("index.jsp?estado=noValido");
		}
	}
}
