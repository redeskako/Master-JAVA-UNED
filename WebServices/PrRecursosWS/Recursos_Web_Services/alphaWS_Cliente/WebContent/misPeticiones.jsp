...
<libTemplate:ilegal rolIlegal="0">
	...
	<div id="container">
		...
		<div id="content">
			...
			<!-- Se recupera el array de las peticiones pendientes -->
			<c:set var="misPeticiones" value="<%= (ArrayList<Reserva>) request.getAttribute(\"arrayReservas\") %>"></c:set>
			...
			<c:if test="${ reservas.size() != 0 }">
				<table>
					...
					<c:forEach var="res" items="${ misPeticiones }">
						<tr>
							...
							<td class="boton">	
								<form action="./FormularioEditarReserva" method="get">
									<input type="hidden" name="id" value="${res.getIdReserva()}">
									<input class="btn" type="submit" name="editar" value="Editar">
								</form>												
							</td>
							<td class="boton">						
								<form action="http://localhost:8080/alphaWS_Servidor/rest/eliminar" method="post">
									<input type="hidden" name="volver" value="http://localhost:8080/alphaWS_Cliente/MisPeticiones">
