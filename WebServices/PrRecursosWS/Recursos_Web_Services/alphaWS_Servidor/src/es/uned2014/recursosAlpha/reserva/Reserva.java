...
public class Reserva implements Comparable<Object>{
	...
	/**
	 * Método constructor, asigna valor a las variables de la tabla Reservas en BBDD:
	 * @param idres para idReserva
	...
	/**
	 * Método constructor, asigna valor a las variables idSucursal y sucursal:
	 * @param s para  idSucursal
	 * @param suc para sucursal
	 */
	...
	/**
	 * Se obtiene el número total de filas de la tabla de reservas de un usuario concreto y
	 * con un estado concreto.
	 * @param idEstado que se desea consultar
	 * @param usuario
	 * @return número de filas con ese estado
	 * @throws null
	...
	public int totalFilasReserva(int idEstado, String usuario ){
		...
		// String con la consulta: Reservas con IdEstado i
		String sql = " SELECT COUNT(*) FROM `reservas` WHERE IdEstado = '" + idEstado + "' AND IdUsuario = '"+ id +"' ";
	 ...
	 * Crea un objeto BaseDatos, abre la conexión y solicita que se ejecute 
	 * una consulta antes de cerrar.
	 * Esta consulta obtiene todas las reservas con un estado concreto.
	 * @param pagina
	 ...
	 * Crea un objeto BaseDatos, abre la conexión y solicita que se ejecute 
	 * una consulta antes de cerrar.
	 * Esta consulta obtiene todas las sucursales posibles para reservas.
	...
	public int nuevo(int usu, int rec, Date i, Date f, int est, int suc){
		...
		// Si el usuario no existe previamente, se crea. Si ya existe, se envÃ­a error.
		if (comprobada){
			BaseDatos bbdd = new BaseDatos();
			bbdd.abrirConexion();
	 ...
	 * @param res para idReserva (no tiene que comprobar que sea compatible consigo misma)
	 * @param rec para idRecurso
	 * @param i para fecha inicio
	 * @param f para fecha fin
	 ...
	 * @param idU para id del usuario
	 * @param idRec para id del recurso
	 * @param i para fecha de inicio
	 * @param f para fecha de fin
	 * @param idE para id del estado3
	 * @param idS para id de la sucursal
	 * @return id de la reserva editada
	 * @throws null
	 */
	public int editarReserva(int id, int idU, int idRec, Date i, Date f, int idE, int idS){
		int idReserva = id;
		int idUsuario = idU;
		int idRecurso = idRec;
		Date inicio = i;
		...
		// Si la reserva es compatible, se modifica. Si ya existe, se envÃ­a error.
		if (comprobada){
			String fi = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(inicio);
			String ff = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(fin);
	 ...
	 * @param est para id de estado (denegar, anular, etc)
	 * @return id de la reserva editada
	 * @throws null
	 */
	 ...
	 * pendiente de anulación), que requiere comprobar compatibilidad.
	 * @param idReserva de la reserva
	 * @param idRecurso para id del Recurso
	 * @param i para fecha de inicio
	 * @param f para fecha de finalización
	 * @param est para id de estado (cancelar,confirmar, etc)
	 * @return id de la reserva editada
	 * @throws null
	 */
	...
	/**
	 * Crea un objeto BaseDatos, abre la conexión y solicita que se ejecute una consulta 
	 * antes de cerrar.
	 * Esta consulta elimina una fila en la tabla de Reservas de la base se datos.
	 * @param i para id del elemento a eliminar
	 ...
	 * @param u idUsuario
	 * @param r idRecurso
	 * @param iniD día de inicio
	 * @param iniH hora de inicio
	 * @param finD día de finalización
	 * @param finH hora de finalización
	 * @param e idEstado
	 * @param s idSucursal
	 * @return sql de la consulta
	 */
	public String sqlBuscadorReservas(int vista, int u, int r, Date iniD, Date iniH, Date finD, 
			Date finH, int e, int s){
		
		// Se obtiene una fecha "vacÃ­a" para poder comprobar si se han rellenado inicio y/o fin 
		// en el formulario:
		...
		if (u != 0){
			...
			sql += " WHERE res.IdUsuario = " + u;
				...
				// Si el nombreUsuario estÃ¡ vacÃ­o en el formulario, se busca en todos los nombreUsuario
				// Número de usuarios:
	...
	public String toString(){
		String s = "IdReserva: " + this.idReserva + " | IdUsuario: " + this.idUsuario + " | IdRecurso: " 
				...
				+ this.descripcionRecurso +  " | Estado: " + this.estado +  " | Sucursal: " + this.sucursal;
