...
/**
 * Servlet implementation class MisPeticionesReservas
 * Gestiona la visualización por parte de un usuario-empleado de sus propias 
 * peticiones (reservas pendientes) y de sus reservas vigentes (reservas 
 * confirmadas, pendientes de anulación y no anuladas).
 * 
...
public class MisPeticionesReservas extends HttpServlet {
	 ...
	 * (peticiones o reservas) y se las hace llegar al jsp para que muestre el listado.
	 * @param request 
	 * @param response
	 * @return null
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		// Por defecto, página 1. Si la sesión tiene asociada otra página ("p"), se asigna ese valor:
		Integer pagina = 1;
		if(p != null){
		...
		case "peticiones":
			...
			ArrayList<Reserva> arrayPeticiones = r.misPeticiones(usuario, pagina);
			...
			RequestDispatcher rd = request.getRequestDispatcher("misPeticionesPendientes.jsp");
		...
		case "reservas":
			...
			int numFilas2 = r.totalFilasReserva(2, usuario) + r.totalFilasReserva(4, usuario) 
			...
			// Se re-dirige a la página jsp:
			RequestDispatcher rd2 = request.getRequestDispatcher("misReservasPendientes.jsp");
		...
		case "solicitarAnulacion":
			...
			if (usuarioAnular){
				// Se solicita el cambio de estado a denegado (3) y se almacena el id:
				idSolicitado = r.cambiarEstado(idSolicitar, 4);
				
				// Si el nuevo id es 0, ha habido un error al editar. Si no, todo ha ido bien.
				response.sendRedirect("./MisPeticionesReservas?tipo=reservas&idEditado="+idSolicitado);
			} else {
		...
		case "botonEliminar":
			...
			// Lo primero, se comprueba si el usuario es el dueño de la reserva:
			boolean usuarioEliminar = r.comprobarUsuario(usuario, Integer.parseInt(idListaEliminar));
		...
		case "eliminar":
			...
			if (usuarioEliminar2){
				...
				// Si el nuevo id es 0, ha habido un error al editar. Si no, todo ha ido bien.
				response.sendRedirect("./MisPeticionesReservas?tipo=peticiones&eliminado=" + eliminado);
		...
		case "botonEditar":
			...
			// Lo primero, se comprueba si el usuario es el dueño de la reserva:
			boolean usuarioEditar = r.comprobarUsuario(usuario, Integer.parseInt(idListaEditar));

			// Se re-dirige:
			if (usuarioEditar){
				response.sendRedirect("./MisPeticionesReservas?tipo=peticiones&idEditar="+idListaEditar);	
			} else {
				response.sendRedirect("index.jsp?estado=ilegal");
			}					
		...
		case "editar":
			...
			String inicioStEdit = request.getParameter("inicio") + " " + request.getParameter("horaInicio")
					+ ":" +  request.getParameter("minutoInicio") + ":00";
			String finStEdit = request.getParameter("final") + " " + request.getParameter("horaFinal")
					+ ":" +  request.getParameter("minutoFinal") + ":00";
			int estadoIdEdit = Integer.parseInt(request.getParameter("estado"));
			int sucursalIdEdit = Integer.parseInt(request.getParameter("sucursal"));
			...
			if (usuarioEditar2){
				...
				try {	 
					...
					idEditado =  r.editarReserva(idReserva, usuIdEdit, recIdEdit, inicioEdit, 
						finEdit, estadoIdEdit, sucursalIdEdit);
				...
				// Si el nuevo id es 0, ha habido un error al editar. Si no, todo ha ido bien.
				response.sendRedirect("./MisPeticionesReservas?tipo=peticiones&idEditado="+idEditado);
			} else {
				response.sendRedirect("index.jsp?estado=ilegal");
			}			
