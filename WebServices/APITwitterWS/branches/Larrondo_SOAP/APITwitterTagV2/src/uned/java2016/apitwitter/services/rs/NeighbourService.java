...
public NeighbourService(){}
	...
	public Response getNeighbour(@PathParam("ht") String ht){
		...
		ArrayList<ClinicalStudy> list=ndao.selectClinicalStudies(ht);
		n=new Clinicals(list);					
		ret=Response.ok(new JAXBElement(new QName("","neighbours"),Clinicals.class,n), MediaType.APPLICATION_XML).build();		
		}catch(Exception e){
