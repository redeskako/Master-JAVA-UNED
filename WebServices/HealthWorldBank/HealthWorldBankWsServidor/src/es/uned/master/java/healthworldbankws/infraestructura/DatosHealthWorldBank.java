 ...
 * 
 */
package es.uned.master.java.healthworldbankws.infraestructura;
...
public class DatosHealthWorldBank {
	...
	private String cadenaDeConexion() {
		if (cadenaDeConexion == null) {
			...
			posible = base + "/webapps/HealthWorldBankWsServidor/resources/Datos.db";
			//Depuramos:
			System.out.println("Posible ruta del fichero de la Base de Datos: " + posible);
			fichero = new File(posible);
			if (fichero.exists()) {
				cadena = posible;
			}

			posible = base + "/wtpwebapps/HealthWorldBankWsServidor/resources/Datos.db";
			//Depuramos:
			System.out.println("Posible ruta del fichero de la Base de Datos: " + posible);
			fichero = new File(posible);
			if (fichero.exists()) {
			...
			cadena = "jdbc:sqlite:" + cadena;
			//Depuramos:
			System.out.println("Cadena de conexion utilizada: " + cadena);
	...
	public Respuesta getObtenerDatos() throws SQLException,Exception{
		...
		case PAISES:
			...
			if(intTotales>0){
				...
				respu = new Respuesta(TipoPeticion.PAISES, preg.getLimite(), preg.getPrimerRegistro(), intTotales, Datos);
			}else
		...
		case INDICADORES:
			...
			if(intTotales>0){
				...
				respu = new Respuesta(TipoPeticion.INDICADORES, preg.getLimite(), preg.getPrimerRegistro(), intTotales, Datos);
			}else
		...
		case ESTADISTICAS:
			...
			if(intTotales>0){
				...
				respu = new Respuesta(TipoPeticion.ESTADISTICAS, preg.getLimite(), preg.getPrimerRegistro(), intTotales, Datos);
			}else
