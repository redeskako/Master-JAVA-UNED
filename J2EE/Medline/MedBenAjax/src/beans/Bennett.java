...
public class Bennett implements Serializable {
	...
	public static ArrayList<Bennett> listadoStartBennett(String palabra,int limit) throws Exception{
		ArrayList<Bennett> miLista = new ArrayList<Bennett>();
		BBDD miBd = new BBDD();
		
		miBd.abrirConexion(); 
		miLista = miBd.listadoStartBennettS(palabra, limit);
		miBd.cerrarConexion();
		return miLista;		
	}
	
	/**
	 * Método que devuelve las 6 primeras ciudades que empiezan por la palabra
	 * pasada como parámetro. 
	 * @param palabra: String -> Palabra que se busca.
	 * @return lista: ArrayList<Bennett> -> Lista de registros para mostrar
	 */
	public static ArrayList<Bennett> listadoCiudades(String palabra) throws Exception{
		ArrayList<Bennett> miLista = new ArrayList<Bennett>();
		BBDD miBd = new BBDD();
		
		miBd.abrirConexion(); 
		miLista = miBd.listadoBennettPrincipio(1, 6, "City", palabra);
		miBd.cerrarConexion();
		return miLista;		
	}
