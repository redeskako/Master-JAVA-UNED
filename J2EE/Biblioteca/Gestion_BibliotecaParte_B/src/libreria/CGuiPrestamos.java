...
//////////////////////////////////Establecer un combo para actualizar
			...
			JComboBox mislibros= CPrestamos.carga("SELECT Nombre as Nombre from Libros ORDER BY Nombre",this.idioma.idioma());
			this.libro.setCellEditor(new DefaultCellEditor(mislibros));
			
			this.socio= this.tabla.getColumnModel().getColumn(1);
			JComboBox missocios= CPrestamos.carga("SELECT CONCAT(DNI,' - ',Apellidos,' ',Nombre) as Nombre from Socios ORDER BY Apellidos,Nombre",this.idioma.idioma());
			this.socio.setCellEditor(new DefaultCellEditor(missocios));			
		}
	public CGuiPrestamos(String titulo,String local){
		super(titulo,
			          true, //resizable
			          true, //closable
			          true, //maximizable
...
//////////////////////////////////Establecer un combo para actualizar
	...
	public void actualizaIdioma(String local){
		this.idioma= new Config(local);
		this.setTitle(this.idioma.traduce("_ALQUILER"));
		this.txtBoton=new String [4];
		this.txtBoton[0]=this.idioma.traduce("_ACEPTAR");
		this.txtBoton[1]=this.idioma.traduce("_CANCELAR");
		this.txtBoton[2]=this.idioma.traduce("_NUEVO");
